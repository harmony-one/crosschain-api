...
async function deposit() {
  ...
  let response = await contract.methods
    .deposit(web3.utils.toWei("23", "ether"))
    .send({
      from: account.address,
      gas: process.env.ETH_GAS_LIMIT,
      gasPrice: new BN(await web3.eth.getGasPrice()).mul(new BN(1)),
      value: web3.utils.toWei("23", "ether"),
    });
...
async function approveBUSDEthManager(amountInWei) {
  ...
  let transaction = await busdContract.methods
    .approve(process.env.HMY_BUSD_MANAGER_CONTRACT, amountInWei)
    .send({
      from: account.address,
      gas: process.env.ETH_GAS_LIMIT,
      gasPrice: new BN(await web3.eth.getGasPrice()).mul(new BN(1)),
...
async function burnTxn(amountInWei) {
  ...
  const busdJson = require("./abi/BUSDHmyManager.json");
  const busdContract = new web3.eth.Contract(
    busdJson.abi,
